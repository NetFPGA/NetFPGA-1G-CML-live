...
void init_ospf(struct sr_instance* sr) {
...
void init_registers(router_state* rs) {

	int s = open("/dev/nf10", O_RDWR);

	if (s < 0) {
		perror("ERROR opening register device /dev/nf10\n");
		exit(1);
	}
...
void init_libnet(router_state* rs) {
	char* iface_names[4] = {"nf0", "nf1", "nf2", "nf3"};
	int i;
