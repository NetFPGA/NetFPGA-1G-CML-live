...
void init_sd_gpio_port(void);
#define	INIT_PORT()	init_sd_gpio_port()     /* Initialize MMC control port (CS=H, CLK=L, DI=H, DO=in) */
#define DLY_US(n)	udelay(n)	/* Delay n microseconds */

#define PORT        *gpio_io

#define	CS_H()		PORT |= SD_SPI_CS       /* Set MMC CS "high" */
#define CS_L()		PORT &= ~SD_SPI_CS      /* Set MMC CS "low" */
#define CK_H()		PORT |= SD_SPI_CK       /* Set MMC SCLK "high" */
#define	CK_L()		PORT &= ~SD_SPI_CK      /* Set MMC SCLK "low" */
#define DI_H()		PORT |= SD_SPI_DI       /* Set MMC DI "high" */
#define DI_L()		PORT &= ~SD_SPI_DI      /* Set MMC DI "low" */
#define DO			(PORT &	SD_SPI_DO)      /* Test for MMC DO ('H':true, 'L':false) */
