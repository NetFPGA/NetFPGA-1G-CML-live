...
my $cpuhwDest = "../scone/cpuhw";
my $iperfDest = "iperf.sh";
my $telnetDest = "telnet.sh";
...
sub createIperf {
	...
	print IPERF "iperf -c 192.168.$nf1.1 -t 100000 -i 1\n";
...
#
sub createTelnet {
	my $dest = glob($telnetDest);

	print "Generating telnet.sh file...\n\n";

	if (!$dryRun) {
		open TELNET, ">$dest" or die "Couldn't open $dest: $!";
	}
	else {
		open TELNET, ">&STDOUT" or die "Couldn't dup STDOUT: $!";
	}
	print TELNET "#!/bin/sh\n\n";
	print TELNET "# telnet.sh file -- generated $now\n\n";
        print TELNET "gnome-terminal -x bash -c \"telnet 192.168.$nf2.2\"";
...
sub updateRclocal {
	...
	print RC "route add -net 192.168.0.0 netmask 255.255.0.0 gw 192.168.$nf2.2 dev eth1\n";
	print RC "\n";
