...
void write_rtable_to_hw(router_state* rs) {
	...
	for (i = 0; i < ROUTER_OP_LUT_ROUTE_TABLE_DEPTH; ++i) {
		...
		if (cur) {
			...
			/* write the ip */
			writeReg(rs->netfpga_regs, ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_IP_REG, ntohl(entry->ip.s_addr));
			/* write the mask */
			writeReg(rs->netfpga_regs, ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG, ntohl(entry->mask.s_addr));
			/* write the next hop */
			writeReg(rs->netfpga_regs, ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG, ntohl(entry->gw.s_addr));
			/* write the port */
			writeReg(rs->netfpga_regs, ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_OUTPUT_PORT_REG, getOneHotPortNumber(entry->iface));
			/* write the row number */
			writeReg(rs->netfpga_regs, ROUTER_OP_LUT_ROUTE_TABLE_WR_ADDR_REG, i);
			...
			/* write the ip */
			writeReg(rs->netfpga_regs, ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_IP_REG, 0);
			/* write the mask */
			writeReg(rs->netfpga_regs, ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG, 0);
			/* write the next hop */
			writeReg(rs->netfpga_regs, ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG, 0);
			/* write the port */
			writeReg(rs->netfpga_regs, ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_OUTPUT_PORT_REG, 0);
			/* set the row */
			writeReg(rs->netfpga_regs, ROUTER_OP_LUT_ROUTE_TABLE_WR_ADDR_REG, i);
		}
	}
