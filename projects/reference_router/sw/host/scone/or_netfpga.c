...
/*
	switch(port) {
		case 0:
			readReg(nf, CPU_REG_Q_0_WR_NUM_PKTS_IN_Q_REG, &retval);
			break;
		case 1:
			readReg(nf, CPU_REG_Q_1_WR_NUM_PKTS_IN_Q_REG, &retval);
			break;
		case 2:
			readReg(nf, CPU_REG_Q_2_WR_NUM_PKTS_IN_Q_REG, &retval);
			break;
		case 3:
			readReg(nf, CPU_REG_Q_3_WR_NUM_PKTS_IN_Q_REG, &retval);
			break;
	}
*/
	return retval;
...
/*
	switch(port) {
		case 0:
			readReg(nf, CPU_REG_Q_0_WR_NUM_WORDS_LEFT_REG, &retval);
			break;
		case 1:
			readReg(nf, CPU_REG_Q_1_WR_NUM_WORDS_LEFT_REG, &retval);
			break;
		case 2:
			readReg(nf, CPU_REG_Q_2_WR_NUM_WORDS_LEFT_REG, &retval);
			break;
		case 3:
			readReg(nf, CPU_REG_Q_3_WR_NUM_WORDS_LEFT_REG, &retval);
			break;
	}
*/
	return retval;
...
/*
	switch(port) {
		case 0:
			retval = writeReg(nf, CPU_REG_Q_0_WR_DATA_WORD_REG, val);
			break;
		case 1:
			retval = writeReg(nf, CPU_REG_Q_1_WR_DATA_WORD_REG, val);
			break;
		case 2:
			retval = writeReg(nf, CPU_REG_Q_2_WR_DATA_WORD_REG, val);
			break;
		case 3:
			retval = writeReg(nf, CPU_REG_Q_3_WR_DATA_WORD_REG, val);
			break;
	}
*/
	assert(!retval);
...
/*	switch(port) {
		case 0:
			retval = writeReg(nf, CPU_REG_Q_0_WR_CTRL_WORD_REG, val);
			break;
		case 1:
			retval = writeReg(nf, CPU_REG_Q_1_WR_CTRL_WORD_REG, val);
			break;
		case 2:
			retval = writeReg(nf, CPU_REG_Q_2_WR_CTRL_WORD_REG, val);
			break;
		case 3:
			retval = writeReg(nf, CPU_REG_Q_3_WR_CTRL_WORD_REG, val);
			break;
	}
*/
	assert(!retval);
...
void* netfpga_stats(void* arg) {
    ...
    gettimeofday(&now, NULL);
		...
		for (i = 0; i < 8; ++i) {
			/* read all the values */
			rx_packets = get_rx_queue_num_pkts_received(&rs->netfpga_regs, i);
			tx_packets = get_tx_queue_num_pkts_sent(&rs->netfpga_regs, i);
			rx_bytes = get_rx_queue_num_bytes_received(&rs->netfpga_regs, i);
			tx_bytes = get_tx_queue_num_bytes_sent(&rs->netfpga_regs, i);
	...
	if (rs->is_netfpga) {
		...
		for (i = 0; i < ROUTER_OP_LUT_DST_IP_FILTER_TABLE_DEPTH; ++i) {
			if (cur) {
				...
				writeReg(rs->netfpga_regs, ROUTER_OP_LUT_DST_IP_FILTER_TABLE_ENTRY_IP_REG, ntohl(entry->ip.s_addr));
				writeReg(rs->netfpga_regs, ROUTER_OP_LUT_DST_IP_FILTER_TABLE_WR_ADDR_REG, i);

				cur = cur->next;
			} else {
				/* zero them out */
				writeReg(rs->netfpga_regs, ROUTER_OP_LUT_DST_IP_FILTER_TABLE_ENTRY_IP_REG, 0);
				writeReg(rs->netfpga_regs, ROUTER_OP_LUT_DST_IP_FILTER_TABLE_WR_ADDR_REG, i);
			}
		}
	}
...
unsigned int get_rx_queue_num_pkts_received(int nf, unsigned char port) {
	...
	switch(port) {
/*		case 0:
			readReg(nf, MAC_GRP_0_RX_QUEUE_NUM_PKTS_STORED_REG, &retval);
			break;
		case 1:
			readReg(nf, MAC_GRP_1_RX_QUEUE_NUM_PKTS_STORED_REG, &retval);
			break;
		case 2:
			readReg(nf, MAC_GRP_2_RX_QUEUE_NUM_PKTS_STORED_REG, &retval);
			break;
		case 3:
			readReg(nf, MAC_GRP_3_RX_QUEUE_NUM_PKTS_STORED_REG, &retval);
			break;
		case 4:
			readReg(nf, CPU_REG_Q_0_RX_NUM_PKTS_RCVD_REG, &retval);
			break;
		case 5:
			readReg(nf, CPU_REG_Q_1_RX_NUM_PKTS_RCVD_REG, &retval);
			break;
		case 6:
			readReg(nf, CPU_REG_Q_2_RX_NUM_PKTS_RCVD_REG, &retval);
			break;
		case 7:
			readReg(nf, CPU_REG_Q_3_RX_NUM_PKTS_RCVD_REG, &retval);
			break;
*/
	}
	return retval;
...
unsigned int get_tx_queue_num_pkts_sent(int nf, unsigned char port) {
	...
	switch(port) {
/*		case 0:
			readReg(nf, MAC_GRP_0_TX_QUEUE_NUM_PKTS_SENT_REG, &retval);
			break;
		case 1:
			readReg(nf, MAC_GRP_1_TX_QUEUE_NUM_PKTS_SENT_REG, &retval);
			break;
		case 2:
			readReg(nf, MAC_GRP_2_TX_QUEUE_NUM_PKTS_SENT_REG, &retval);
			break;
		case 3:
			readReg(nf, MAC_GRP_3_TX_QUEUE_NUM_PKTS_SENT_REG, &retval);
			break;
		case 4:
			readReg(nf, CPU_REG_Q_0_TX_NUM_PKTS_SENT_REG, &retval);
			break;
		case 5:
			readReg(nf, CPU_REG_Q_1_TX_NUM_PKTS_SENT_REG, &retval);
			break;
		case 6:
			readReg(nf, CPU_REG_Q_2_TX_NUM_PKTS_SENT_REG, &retval);
			break;
		case 7:
			readReg(nf, CPU_REG_Q_3_TX_NUM_PKTS_SENT_REG, &retval);
			break;
*/
	}
	return retval;
...
unsigned int get_rx_queue_num_bytes_received(int nf, unsigned char port) {
	...
	switch(port) {
/*		case 0:
			readReg(nf, MAC_GRP_0_RX_QUEUE_NUM_BYTES_PUSHED_REG, &retval);
			break;
		case 1:
			readReg(nf, MAC_GRP_1_RX_QUEUE_NUM_BYTES_PUSHED_REG, &retval);
			break;
		case 2:
			readReg(nf, MAC_GRP_2_RX_QUEUE_NUM_BYTES_PUSHED_REG, &retval);
			break;
		case 3:
			readReg(nf, MAC_GRP_3_RX_QUEUE_NUM_BYTES_PUSHED_REG, &retval);
			break;
		case 4:
			readReg(nf, CPU_REG_Q_0_RX_NUM_BYTES_RCVD_REG, &retval);
			break;
		case 5:
			readReg(nf, CPU_REG_Q_1_RX_NUM_BYTES_RCVD_REG, &retval);
			break;
		case 6:
			readReg(nf, CPU_REG_Q_2_RX_NUM_BYTES_RCVD_REG, &retval);
			break;
		case 7:
			readReg(nf, CPU_REG_Q_3_RX_NUM_BYTES_RCVD_REG, &retval);
			break;
*/
	}
	return retval;
...
unsigned int get_tx_queue_num_bytes_sent(int nf, unsigned char port) {
	...
	switch(port) {
/*		case 0:
			readReg(nf, MAC_GRP_0_TX_QUEUE_NUM_BYTES_PUSHED_REG, &retval);
			break;
		case 1:
			readReg(nf, MAC_GRP_1_TX_QUEUE_NUM_BYTES_PUSHED_REG, &retval);
			break;
		case 2:
			readReg(nf, MAC_GRP_2_TX_QUEUE_NUM_BYTES_PUSHED_REG, &retval);
			break;
		case 3:
			readReg(nf, MAC_GRP_3_TX_QUEUE_NUM_BYTES_PUSHED_REG, &retval);
			break;
		case 4:
			readReg(nf, CPU_REG_Q_0_TX_NUM_BYTES_SENT_REG, &retval);
			break;
		case 5:
			readReg(nf, CPU_REG_Q_1_TX_NUM_BYTES_SENT_REG, &retval);
			break;
		case 6:
			readReg(nf, CPU_REG_Q_2_TX_NUM_BYTES_SENT_REG, &retval);
			break;
		case 7:
			readReg(nf, CPU_REG_Q_3_TX_NUM_BYTES_SENT_REG, &retval);
			break;
*/
	}
	return retval;
...
unsigned int get_rx_queue_num_pkts_dropped_full(int nf, unsigned char port) {
	...
	switch(port) {
/*		case 0:
			readReg(nf, MAC_GRP_0_RX_QUEUE_NUM_PKTS_DROPPED_FULL_REG, &retval);
			break;
		case 1:
			readReg(nf, MAC_GRP_1_RX_QUEUE_NUM_PKTS_DROPPED_FULL_REG, &retval);
			break;
		case 2:
			readReg(nf, MAC_GRP_2_RX_QUEUE_NUM_PKTS_DROPPED_FULL_REG, &retval);
			break;
		case 3:
			readReg(nf, MAC_GRP_3_RX_QUEUE_NUM_PKTS_DROPPED_FULL_REG, &retval);
			break;
*/
	}
	return retval;
...
unsigned int get_rx_queue_num_pkts_dropped_bad(int nf, unsigned char port) {
	...
	switch(port) {
/*		case 0:
			readReg(nf, MAC_GRP_0_RX_QUEUE_NUM_PKTS_DROPPED_BAD_REG, &retval);
			break;
		case 1:
			readReg(nf, MAC_GRP_1_RX_QUEUE_NUM_PKTS_DROPPED_BAD_REG, &retval);
			break;
		case 2:
			readReg(nf, MAC_GRP_2_RX_QUEUE_NUM_PKTS_DROPPED_BAD_REG, &retval);
			break;
		case 3:
			readReg(nf, MAC_GRP_3_RX_QUEUE_NUM_PKTS_DROPPED_BAD_REG, &retval);
			break;
*/
	}
	return retval;
...
unsigned int get_oq_num_pkts_dropped(int nf, unsigned char port) {
	...
	switch(port) {
/*		case 0:
			readReg(nf, OQ_QUEUE_0_NUM_PKTS_DROPPED_REG, &retval);
			break;
		case 1:
			readReg(nf, OQ_QUEUE_1_NUM_PKTS_DROPPED_REG, &retval);
			break;
		case 2:
			readReg(nf, OQ_QUEUE_2_NUM_PKTS_DROPPED_REG, &retval);
			break;
		case 3:
			readReg(nf, OQ_QUEUE_3_NUM_PKTS_DROPPED_REG, &retval);
			break;
		case 4:
			readReg(nf, OQ_QUEUE_4_NUM_PKTS_DROPPED_REG, &retval);
			break;
		case 5:
			readReg(nf, OQ_QUEUE_5_NUM_PKTS_DROPPED_REG, &retval);
			break;
		case 6:
			readReg(nf, OQ_QUEUE_6_NUM_PKTS_DROPPED_REG, &retval);
			break;
		case 7:
			readReg(nf, OQ_QUEUE_7_NUM_PKTS_DROPPED_REG, &retval);
			break;
*/
	}
	return retval;
