...
static uint32_t simple_rd(struct nf10_card *card, uint32_t address){
    uint64_t val;
    uint32_t rd_value;

    *(((uint64_t*)card->cfg_addr) + 129) = ((uint64_t)address << 32);
    mb();
    val = *(((uint64_t*)card->cfg_addr) + 129);
    rd_value = (uint32_t)(val & 0xffffffff);

    return rd_value;
}

static void simple_wr(struct nf10_card *card, uint32_t address, uint32_t value){
    uint64_t buff_info;

    //check buffer no full
    do {
        buff_info = *(((uint64_t*)card->cfg_addr) + 130);
        mb();
    } while( !(buff_info & 0x1) && !(~(buff_info & 0x2)) && !(~(buff_info & 0x4)));

    *(((uint64_t*)card->cfg_addr) + 128) = ((uint64_t)address << 32) | value;
...
static int XEmacLite_PhyRead(struct nf10_card *card, uint32_t PhyAddress, uint32_t RegNum, uint32_t opcode, uint32_t clause, uint16_t *PhyDataPtr) {
    ...
    if ((simple_rd(card, rd_address) & XEL_MDIOCNTR_STATUS_MASK)  != 0) {
        rd_value = simple_rd(card, rd_address);
        return -1;
    }
    ...
    simple_wr(card, wr_address, PhyAddrReg);

    //Enable MDIO and start the transfer.
    rd_address = MDIO_BASE_ADDR + XEL_MDIOCNTR_OFFSET;
    rd_value = simple_rd(card, rd_address);
    wr_address = MDIO_BASE_ADDR + XEL_MDIOCNTR_OFFSET;
    simple_wr(card, wr_address, rd_value | XEL_MDIOCNTR_STATUS_MASK | XEL_MDIOCNTR_ENABLE_MASK);

    //Wait till the completion of transfer.
    rd_address = MDIO_BASE_ADDR + XEL_MDIOCNTR_OFFSET;
    ael2005_sleep(2);
    while ((simple_rd(card, rd_address) & XEL_MDIOCNTR_STATUS_MASK) != 0);

    //Read data from MDIO read data register.
    rd_address = MDIO_BASE_ADDR + XEL_MDIORD_OFFSET;
    *PhyDataPtr = (uint16_t)simple_rd(card, rd_address);

    //Disable the MDIO.
    rd_address = MDIO_BASE_ADDR + XEL_MDIOCNTR_OFFSET;
    rd_value = simple_rd(card, rd_address);
    wr_address = MDIO_BASE_ADDR + XEL_MDIOCNTR_OFFSET;
    simple_wr(card, wr_address, rd_value & ~XEL_MDIOCNTR_ENABLE_MASK);
...
static int XEmacLite_PhyWrite(struct nf10_card *card, uint32_t PhyAddress, uint32_t RegNum, uint32_t opcode, uint32_t clause, uint16_t PhyData) {
    ...
    simple_wr(card, wr_address, (uint32_t)PhyData);
