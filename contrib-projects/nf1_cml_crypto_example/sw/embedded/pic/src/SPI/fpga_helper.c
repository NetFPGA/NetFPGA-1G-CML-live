...
/*!
 * \brief Initializes the FPGA to SPI channel.
 *
 * Does nothing, initializing is handled in utility.c
 *
 * \param[in] channel the SPI channel to initialize
 */
...
/*!
 * \brief Checks a SPI channel to see if there is data available.
 *
 * \param[in] channel the SPI channel to check
 *
 * \return a data byte from the SPI channel if applicable, otherwise NO_MSG byte
 */
...
/*!
 * \brief Sends a data ACK byte to the Microblaze over a SPI channel.
 *
 * \param[in] channel the SPI channel to send the ACK over
 * \param[in] msg the ACK byte value to send
 */
...
/*!
 * \brief Sends a byte buffer to the FPGA over a SPI channel
 *
 * \param[in] channel the SPI channel to send the data over
 * \param[in] msg a pointer to a message buffer
 * \param[in] len the length of the message buffer in bytes
 * \param[in] status a status byte to send with the message
 */
...
/*!
 * \brief Dispatches a request byte from the Microblaze (performs the specified
 * command).
 *
 * \param[in] req the command byte
 *
...
//                                             show_RTC_val(&rtc_t);
        ...
        case RTC_WRITE_TIME:
                                            ...
                                            break;
        default:
                                            printf("Unrecognized request, no handler implemented: 0x%02X\r\n", req);
                                            break;
  }
}

/*!
 * \brief Prints the command associated with the request byte from the Microblaze.
 *
 * \param[in] request the request byte from the Microblaze
 */
#if (1 == VERBOSE)
void show_FPGA_request(UINT8 request)
{
...
/*!
 * \brief Displays the contents of an RTC_TIME structure.
 *
 * \param time a pointer to the RTC_TIME structure to display
 */
#if (1 == DEBUG)
void show_RTC_val(RTC_TIME *time)
{
...
/*!
 * \brief Displays the status of a SPI channel.
 *
 * \param[in] spi_chan the SPI channel to display the status of
 */
void dump_SPI_status(SpiChannel spi_chan)
{
