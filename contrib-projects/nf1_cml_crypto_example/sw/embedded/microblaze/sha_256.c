...
static u32 Sum_0_256(u32 x);
static u32 Sum_1_256(u32 x);
static u32 Theta_0_256(u32 x);
static u32 Theta_1_256(u32 x);
static void InitWordSchedule(u32 *W, u8 *padded);
...
//			MAKE_UINT(W[i], padded, i * 4);
            tmp1 = ((u32)padded[i*4]) << 24;
            tmp2 = ((u32)padded[i*4+1]) << 16;
            tmp3 = ((u32)padded[i*4+2]) << 8;
            tmp4 = ((u32)padded[i*4+3]);
...
{
	int i, ii, top;
    u32 full_message_len;
    u32 num_extra_blocks;
	u8 *bit_len_p = (u8 *) &full_message_len;
