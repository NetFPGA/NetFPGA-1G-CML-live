...
void write_arp_cache_entry_to_hw(router_state* rs, arp_cache_entry *entry, int row) {
	...
	if(entry != NULL) {
		...
		writeReg(rs->netfpga_regs, ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_HI_REG, mac_hi);
		...
		writeReg(rs->netfpga_regs, ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_LO_REG, mac_lo);

		/* write the next hop ip data */
		writeReg(rs->netfpga_regs, ROUTER_OP_LUT_ARP_TABLE_ENTRY_NEXT_HOP_IP_REG, ntohl(entry->ip.s_addr));
